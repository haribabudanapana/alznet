...
import filterTestData from '@data/Staging/alznetdashboard/e2e/apply-sex-at-birth-filters-data.json' with { type: 'json' };
        ...
        if (testData.category === "Demographics") {
            test(`should correctly apply all filter variations for ${testData.category}`, async () => {
                ...
                await test.step(`Select one option from ${filterName}`, async () => {
                    ...
                    await exploreDataPage.selectAnyOne(filterName, anyOption);
