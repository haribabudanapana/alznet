...
test.describe('Site User > Submit & View Data Change Request', () => {
  test('Site User > Submit & View Data Change Request', async ({
    loginPage,
    ...
    // Step 2 & 3
    await caseListPage.openCaseList();
    await caseListPage.applyFiletrsOnFirstNameAndClickChangeRequest(dynamicFirstName); // Filter by first name and click change request
    //Verify Data Change Request Form opens with fields already filled with case details.
    const caseId = await caseListPage.verifyFilledCaseDeatilsInChangeRequestForm(ENV.TAU_APPUSERNAME);
    await caseListPage.fillChangeRequestDetails(testData.changecase.FormType, testData.changecase.changeReason, testData.changecase.selectReasonForChange);
    await caseListPage.validatebackToCaseListNavigation(testData.backNvaigationPopupText, testData.proceedBtn);

    await caseListPage.openCaseList();
    await caseListPage.applyFiletrsOnFirstNameAndClickChangeRequest(dynamicFirstName);
    await caseListPage.fillChangeRequestDetails(testData.changecase.FormType, testData.changecase.changeReason, testData.changecase.selectReasonForChange);
    await caseListPage.submitChangeRequestForm();
    await caseListPage.verifyChangeRequestSubmissionSuccessAndClose();
    await changeRequestListPageFixture.verifyChangeRequest(caseId);
