...
import testData from '@data/Staging/alznetapplication/smoke/ALZ-T143.json' with { type: 'json' };
...
test.describe('Site Users > Clinical Imaging Submission Form', () => {
  test('Site Users > Clinical Imaging Submission Form', async ({
    loginPage,
    homePage, // Use the original homePage fixture
    caseListPage, // Use the original siteFeasibilityPage fixture
    registrationFormPage: registrationFormPageFixture,
    changeRequestListPage: changeRequestListPageFixture, // Rename fixture to avoid conflict with local variable
    clinicalImagingSubmissionPage,
    clinicalImagingSubmissionFormPage,
    imagingLogPage
  }) => {
    const appUrl = ENV.APP_URL;
    const landingPageUrl = ENV.LANDING_PAGE_URL;
    ...
    const dynamicTenDigitNumber = faker.number.int({ min: 1000000000, max: 9999999999 }).toString();
    ...
    // await caseListPage.openCaseList();
    // await caseListPage.applyFiltersOnFirstNameAndClickMenu(testData.firstName, testData.menuItem); // Filter by first name and click change request
    // await clinicalImagingSubmissionPage.fillImagingLogAndSubmit(testData.imagingLogData, dynamicTenDigitNumber);
    // await clinicalImagingSubmissionPage.clearFormButton();
    // await clinicalImagingSubmissionFormPage.applyFilterAndOpenDetails(dynamicTenDigitNumber);
    await loginPage.goto(landingPageUrl);
    await homePage.clickAccessMyFacilityAndVerify(testData.imagingLogData.imagingFacility);
    registrationFormPage = await homePage.accessSavedDraftRegistrationForm(testData.imagingLogData.imagingFacility);
    await registrationFormPage.clickImagingLogTab();
   // imagingLogPage.verifyTriadLaunchOpensNewTab();
    imagingLogPage.applyFiltersOnImageAccessionAndClickMenu(testData.imagingLogData.subjectId);
    imagingLogPage.verifyColumnHeaderLocators();
    imagingLogPage.editAndSaveDicomImageDataupload();
